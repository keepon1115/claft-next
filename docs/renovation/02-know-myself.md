...
3. **であった人ずかん** → `/myself/zukan`(「出会いのコレクション」— フェーズ11-Aで実装。それまでカードは「準備中」)
4. **ストーリー** → `/myself/story`(「きみの冒険の書」)
5. **PBL** → `/pbl`(「探究の記録」)
...
- 計測は外部(Ai GROW)で実施済み。**成果物PDFを表示するだけ**の仕組みでよい
- 併せて、面談で確定した気質5項目(`member_traits.temperament`、11-F参照)があれば
  PDF上部にバー表示(本人のみ閲覧)。無ければPDFのみでよい
